...
func AutoConfigure(options ...Option) boot.AutoConfiguration {
	...
	return boot.NewAutoConfiguration(
		StarterID,
		func(_ context.Context, app *appcontext.ApplicationContext) error {
...
func (c configuration) RegisterWithContext(
	...
	runtimeProvider := func(
		ctx context.Context,
		_ goarkcontainer.Resolver,
	) (*Runtime, error) {
		...
		runtime := &Runtime{
			context: loggerContext, logger: logger,
			previousDefault: previous, manageLifecycle: resolved.manageLifecycle,
		}
	...
	if err := goarkcontainer.Register[*Runtime](
		config.Registry(), BeanNameLifecycle, runtimeProvider,
	); err != nil {
		return err
	}
	loggerProvider := func(
	...
	if err := goarkcontainer.Register[*slog.Logger](
		config.Registry(), BeanNameLogger, loggerProvider,
		goarkcontainer.WithDependsOn(BeanNameLifecycle), goarkcontainer.WithPrimary(),
	); err != nil {
		return err
	}
	systemProvider := func(
	...
	if err := goarkcontainer.Register[LoggingSystem](
		config.Registry(), BeanNameSystem, systemProvider,
	...
	return goarkcontainer.Register[*log.LoggerContext](
		config.Registry(),
		BeanNameContext,
		func(ctx context.Context, resolver goarkcontainer.Resolver) (*log.LoggerContext, error) {
			runtime, err := goarkcontainer.Get[*Runtime](ctx, resolver, BeanNameLifecycle)
			if err != nil {
				return nil, err
			}
		...
		goarkcontainer.WithDependsOn(BeanNameLifecycle),
		goarkcontainer.WithLifecycleManaged(false),
	)
}
